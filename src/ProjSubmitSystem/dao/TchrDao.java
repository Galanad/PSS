...
import ProjSubmitSystem.beans.TchrBean;
...
public class TchrDao {
    ...
    public List getRecordsDataBySql(String sql) {
        ...
        try {
            ...
            while (rs.next()) {
                TchrBean tchrBean = new TchrBean();
                // 索引是从1开始的
                tchrBean.setTno(rs.getString(1));
                tchrBean.setTname(rs.getString(2));
                tchrBean.setTpro(rs.getString(3));
                tchrBean.setTcode(rs.getString(4));
                tchrBean.setTemail(rs.getString(5));
                list.add(tchrBean);
            }
        } catch (Exception e) {
            e.printStackTrace();
