...
import ProjSubmitSystem.beans.WkBean;
...
public class WkDao {
    ...
    public List getRecordsDataBySql(String sql) {
        ...
        try {
            ...
            while (rs.next()) {
                WkBean wkBean = new WkBean();

                wkBean.setWno(rs.getString(1));
                wkBean.setPno(rs.getString(2));
                wkBean.setSno(rs.getString(3));
                wkBean.setScore(rs.getBigDecimal(4));
                wkBean.setWurl(rs.getString(5));
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
