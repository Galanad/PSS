...
import ProjSubmitSystem.beans.ProjBean;
    ...
    public List getRecordsDataBySql(String sql) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // 索引是从1开始的
                projBean.setPno(rs.getString(1));
                projBean.setPname(rs.getString(2));
                projBean.setCno(rs.getString(3));
                projBean.setPurl(rs.getString(4));
                projBean.setPdmd(rs.getString(5));
                projBean.setPa(rs.getInt(6));
                projBean.setPddl(rs.getDate(7));
                list.add(projBean);
            }
        } catch (Exception e) {
            e.printStackTrace();
