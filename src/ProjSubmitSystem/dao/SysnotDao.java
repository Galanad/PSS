...
import ProjSubmitSystem.beans.SysnotBean;
...
public class SysnotDao {
    ...
    public List getRecordsDataBySql(String sql) {
        ...
        try {
            ...
            while (rs.next()) {
                SysnotBean sysnotBean = new SysnotBean();
                // 索引是从1开始的
                sysnotBean.setSyntno(rs.getString(1));
                sysnotBean.setSyntcont(rs.getString(2));
                list.add(sysnotBean);
            }
        } catch (Exception e) {
            e.printStackTrace();
